function featvect=feature_vect_test_logan(syllable,Fs)

% featvect=FEATURE_VECT(syllable)
%
% Returns a row vector which is the feature vector of a particular syllable.
% use a limited number of features, hopefully ones that are indendepent of loudness, to try to label syllables
   ...
   featvect(1)=sum(sd.*sdf)/100;    % First: The Mean Frequency; dependent on loudness?
   ...
   featvect(3)=Dur*1000;             % Fourth: Duration of Syllable (in ms);
   featvect(4)=-100*sum(loud.*log2(loud))/log2(length(loud));    % Fifth: Entropy of the loudness vs. time 
...
%   featvect(6)=50+50*(sum1-sum2)/(sum1+sum2);  % Amplitude Slope;
   %featvect(11)=100*peakat;   % Location of peak amplitude
   %featvect(12)=10000*circ_mag;
   %featvect(13)=15*log10(spec_peak);                    
   featvect(5)=100*spec_ent;        %spectrotemp entropy; diff between sweep and stack
   
   %featvect(8)=10*log10(peak);                  % eight: Peak Loudness
%   featvect(8)=peak;                  % eight: Peak Loudness
   featvect(6)=meanLoud;                  % nine: Mean Loudness
